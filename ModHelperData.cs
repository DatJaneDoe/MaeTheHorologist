namespace TimeHero;

public static class ModHelperData
{
    public const string WorksOnVersion = "40.0";
    public const string Version = "1.1.0";
    public const string Name = "Mae the Timekeeper";

    public const string Description = "Adds a hero with rewinding projectiles and creates damage tracking timerifts!";

    public const string Icon = "pack1";
    public const string RepoOwner = "DatJaneDoe";
    public const string RepoName = "MaeTheHorologist;
    
}
